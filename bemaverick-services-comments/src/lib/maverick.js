...
module.exports.publishChange = publishChange = (
  { token, contentType, contentId, action },
  clientToken = false,
) =>
  new Promise(async (resolve, reject) => {
    if (token == awsLambdaApiKey) {
    ...
    const options = {
      uri: `${config.maverick.url}/v1/site/publishchange`,
      method: 'GET',
      qs: params,
      json: true,
      headers,
    ...
    console.log('Maverick.publishChange.options', options);
