//
//  PostCoverSelectorViewController.swift
//  BeMaverick
//
//  Created by David McGraw on 2/1/18.
...
class PostCoverSelectorViewController: PostRecordParentViewController {
    ...
    /**
     Advance to final metadata state
     */
    @IBAction func nextButtonTapped(_ sender: Any) {
        ...
        performSegue(withIdentifier: R.segue.postCoverSelectorViewController.unwindSegueToMetadata, sender: self)
    ...
    */
    override func configureView() {
        ...
        /// Set nav title
        titleLabel?.text = cameraManager?.sessionChallengeTitle ?? challengeTitle
    ...
    */
    fileprivate func configureTimeline() {
        
        // Layout Timeline and Filter Preview
        guard let segmentDurations = (cameraManager?.session?.segments.map { CMTimeGetSeconds($0.duration) }) else {
            return
        }
       
        if didLayoutTimeline { return }
        didLayoutTimeline = true
        
        recordingTimelineView.delegate = self
        recordingTimelineView.layoutTimeline(withSegmentDurations: segmentDurations,
                                             maxRecordingLimit: CGFloat(Constants.CameraManagerMaxRecordDuration))
...
extension PostCoverSelectorViewController: UIScrollViewDelegate {
...
extension PostCoverSelectorViewController: PostTimelineDelegate {
    
    func didSelectSegment(atIndex index: Int) {
        
        if index == -1 {
            
            editorVideoComposition?.assetRepresentingExportWithOverlay { asset in
            ...
            guard let asset = editorVideoComposition?.savedOverlayAsset(forIndex: index) else { return }
