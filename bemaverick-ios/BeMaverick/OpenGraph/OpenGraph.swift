...
public struct OpenGraph {
    ...
    static var results : [String : OpenGraph] = [:]
    public static func fetch(url: URL, completion: @escaping (OpenGraph?, Error?) -> Void) {
        
        
        if let og = OpenGraph.results[url.absoluteString] {
        ...
        let task = session.dataTask(with: url, completionHandler: { (data, response, error) in
            handleFetchResult(originalURL: url.absoluteString, data: data, response: response, error: error, callback: completion)
        })
        task.resume()
    }
    ...
    private static func handleFetchResult(originalURL: String?, data: Data?, response: URLResponse?, error: Error?, callback: @escaping (OpenGraph?, Error?) -> Void){
        ...
        case (let data?, let response as HTTPURLResponse, _):
            if !(200..<300).contains(response.statusCode) {
                ...
                if  let key = originalURL {
                    print("🤣 saving url : \(key)")
                    OpenGraph.results[key] = og
