...
';
			...
			if ( apply_filters('fb_og_enabled', true) ) {
				...
				if ( is_singular() ) { //Including homepage if set as static page
					...
					// $sep = ' - ';
					// $fb_title = html_entity_decode(get_the_title(2901)); // Homepage title
					if ( is_single() || ( is_home() && ! is_front_page() ) || ( is_page() && ! is_front_page() ) ) {
						$post_title = html_entity_decode(single_post_title( '', false ));
						$fb_title = $post_title;
					}
					// die('$fb_title => '.$fb_title);
				...
				// Add og title suffix
				if (!is_front_page()) {
					$fb_title = $fb_title.' - '.html_entity_decode(get_the_title(2901)); // Homepage title
				}
