...
if (!function_exists('be_portfolio')) {
	function be_portfolio( $atts ) {
		...
		if ( $the_query->have_posts() ) :
          ...
          $portfolioInfo = get_be_themes_portfolio_info(get_the_ID(), $col, $masonry);
					$attachment_id = get_post_thumbnail_id(get_the_ID());
					$image_atts = get_portfolio_image(get_the_ID(), $col, $masonry);
					$attachment_thumb = wp_get_attachment_image_src( $attachment_id, 'full');
					...
					$output .= '<a href="'.$portfolioInfo['challengeUrl'].'" class=" thumb-wrap '.$thumb_class.' '.$mfp_class.'" title="'.$attachment_info['title'].'" '.$target.'>';
					$output .= '<div class="flip-wrap" ><div ' . ( "masonry_enable" == $masonry_enable ? ( 'data-aspect-ratio="'.$masonry_aspect_ratio.'"' )  : '' ) . '  style = "padding-bottom : '. $placeholder_padding .'%;'.( !empty( $placeholder_color ) ? 'background-color:'. $placeholder_color : '' ).';" class="flip-img-wrap' . ( ( 'none' != $image_effect ) ? ( ' '.$image_effect.'-effect"' ) : '"' ) .'><img '. ( $enable_data_src ? 'data-src="'.$attachment_thumb_url : 'src="'.$attachment_thumb_url ) .'" ' . ( $isdwdh ? ( 'data-aspect-ratio="'.$current_dwdh_aspect_ratio.'"' ) : '' ) . 'alt="'.$attachment_info['alt'].'" class="challenge-card"/></div></div>';
					$output .= '<div class="thumb-title-wrap ">';

          // Portfolio activity info
          $output .= '<div class="portfolio-username">'.$portfolioInfo['username'].'</div>';
          $output .= '<div class="portfolio-activity">';
          $output .= '<div class="activity-container">'.$portfolioInfo['numberComments'].'<div class="comment-icon"></div></div>';
          $output .= '<div class="activity-container">'.$portfolioInfo['numberResponses'].'<div class="response-icon"></div></div>';
          $output .= '</div>';
