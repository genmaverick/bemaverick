...
/* ---------------------------------------------  */
if ( ! function_exists( 'be_themes_get_template_directory_uri' ) ) {
	function be_themes_get_template_directory_uri() {
		$base_uri = get_template_directory_uri();
		// $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https:" : "http:";
		// $protocol = 'https:';
		// $base_uri = $protocol. str_replace( array( 'http:', 'https:' ), '', $base_uri );
		// die($base_uri);
		return $base_uri;
	}
}

// filter url for asset loading without protocol

function be_template_content_url($path) {
	$protocol = '';
	$path = $protocol. str_replace( array( 'http:', 'https:' ), '', $path );
	return $path;
}
add_filter( 'content_url', 'be_template_content_url', 99, 1 );

function be_template_site_url($path) {
	$protocol = '';
	$path = $protocol. str_replace( array( 'http:', 'https:' ), '', $path );
	return $path;
}
add_filter( 'site_url', 'be_template_site_url', 99, 1 );

function be_template_plugins_url($url, $path, $plugin) {
	$protocol = '';
	$url = $protocol. str_replace( array( 'http:', 'https:' ), '', $url );
	return $url;
}
add_filter( 'plugins_url', 'be_template_plugins_url', 10, 3 );
...
$be_custom_font_arr = array(
    ...
    "Proxima Nova Semi Bold" => "Proxima Nova Semi Bold",
    "Proxima Nova Bold" => "Proxima Nova Bold",
    "Proxima Nova Extra Bold" => "Proxima Nova Extra Bold",
...
if ( ! function_exists( 'be_themes_add_styles' ) ) {
	function be_themes_add_styles() {		
		...
		wp_register_style( 'be-style-main-css', be_themes_get_template_directory_uri() . '/css/main' . $suffix . '.css', array(), $theme_version );
		wp_enqueue_style( 'be-style-main-css' );

		if( isset( $sticky_sections ) && !empty( $sticky_sections ) ) {
			wp_register_style( 'be-style-sticky-sections', be_themes_get_template_directory_uri().'/css/vendor/sticky-sections' . $suffix . '.css', array(), $theme_version );
			wp_enqueue_style( 'be-style-sticky-sections' );
		}

		if( 'top' == $be_themes_data['opt-header-type'] ){
			wp_register_style( 'be-style-top-header', be_themes_get_template_directory_uri().'/css/headers/top-header' . $suffix . '.css', array(), $theme_version );
			wp_enqueue_style( 'be-style-top-header' );
		}

		if( be_is_special_left_menu() ){
			wp_register_style( 'be-style-left-header', be_themes_get_template_directory_uri().'/css/headers/left-header' . $suffix . '.css', array(), $theme_version );
			wp_enqueue_style( 'be-style-left-header' );
		}

		wp_register_style( 'be-style-responsive-header', be_themes_get_template_directory_uri().'/css/headers/responsive-header' . $suffix . '.css', array(), $theme_version );
		wp_enqueue_style( 'be-style-responsive-header' );

		if( be_is_special_top_menu( 'page-stack-top' ) ){
			wp_register_style( 'be-style-page-stack-top', be_themes_get_template_directory_uri().'/css/headers/page-stack-top' . $suffix .  '.css', array(), $theme_version );
		...
		if( be_is_special_top_menu( 'page-stack-left' ) || be_is_special_top_menu( 'page-stack-right' ) ){
			wp_register_style( 'be-style-page-stack-left-right', be_themes_get_template_directory_uri().'/css/headers/page-stack-left-right' . $suffix . '.css', array(), $theme_version );
		...
		if( be_is_special_top_menu( 'perspective-left' ) || be_is_special_top_menu( 'perspective-right' ) || be_is_special_left_menu( 'perspective-right' ) ){
			wp_register_style( 'be-style-perspective-left-right', be_themes_get_template_directory_uri().'/css/headers/perspective-left-right' . $suffix . '.css', array(), $theme_version );
		...
		if( be_is_special_top_menu( 'overlay-center-align-menu' ) || be_is_special_top_menu( 'overlay-horizontal-menu' ) || be_is_special_left_menu( 'overlay-center-align-menu' ) || be_is_special_left_menu( 'overlay-left-align-menu' ) ){
			wp_register_style( 'be-style-overlay-center-and-left-align', be_themes_get_template_directory_uri().'/css/headers/overlay-center-and-left-align' . $suffix . '.css', array(), $theme_version );
		...
		if( be_is_special_top_menu( 'special-left-menu' ) || be_is_special_top_menu( 'special-right-menu' ) ){
			wp_register_style( 'be-style-special-left-right-menu', be_themes_get_template_directory_uri().'/css/headers/special-left-right-menu' . $suffix . '.css', array(), $theme_version );
		...
		// if( ( be_is_special_top_menu() && isset( $be_themes_data['top-header-submenu-style'] ) && !empty( $be_themes_data['top-header-submenu-style'] ) ) ||
			// ( be_is_special_left_menu() && isset( $be_themes_data['left-header-submenu-style'] ) && !empty( $be_themes_data['left-header-submenu-style'] ) ) ){
				wp_register_style( 'be-style-multilevel-menu', be_themes_get_template_directory_uri().'/css/headers/multilevel-menu' . $suffix . '.css', array(), $theme_version );
				wp_enqueue_style( 'be-style-multilevel-menu' );
		// }

		wp_register_style( 'be-themes-layout', be_themes_get_template_directory_uri().'/css/layout' . $suffix . '.css', array(), $theme_version );
		wp_enqueue_style( 'be-themes-layout' );	
	
		wp_deregister_style( 'oshine_icons' );
		wp_register_style( 'oshine_icons', be_themes_get_template_directory_uri().'/fonts/icomoon/style' . $suffix . '.css', array(), $theme_version );
		wp_enqueue_style( 'oshine_icons' );	

		wp_deregister_style( 'magnific-popup' );
		if( empty( $suffix ) ) {
			wp_register_style( 'magnific-popup', be_themes_get_template_directory_uri().'/css/vendor/magnific-popup.css' );
			wp_enqueue_style( 'magnific-popup' );

			wp_register_style( 'scrollbar', be_themes_get_template_directory_uri().'/css/vendor/scrollbar.css' );
			wp_enqueue_style( 'scrollbar' );

			wp_register_style( 'flickity', be_themes_get_template_directory_uri().'/css/vendor/flickity.css' );
			wp_enqueue_style( 'flickity' );		
		} else {
			wp_register_style( 'vendor', be_themes_get_template_directory_uri() . '/css/vendor/vendor.min.css', array(), $theme_version );
			wp_enqueue_style( 'vendor' );
		}

		// wp_register_style( 'be-animations', be_themes_get_template_directory_uri().'/css/animate-custom.css' );
		// wp_enqueue_style( 'be-animations' );

		// wp_register_style( 'be-slider', be_themes_get_template_directory_uri().'/css/be-slider.css' );
		// wp_enqueue_style( 'be-slider' );

		wp_register_style( 'be-custom-fonts', be_themes_get_template_directory_uri().'/fonts/fonts' . $suffix . '.css', array(), $theme_version );
...
if ( ! function_exists( 'be_themes_add_scripts' ) ) {
	function be_themes_add_scripts() {
		...
		$suffix = ( ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) || !$minified_assets ) ? '' : '.min';
		wp_register_script( 'modernizr', be_themes_get_template_directory_uri() . '/js/vendor/modernizr' . $suffix . '.js', '2.6.2', false );
		wp_enqueue_script( 'modernizr' );
		
		wp_register_script( 'asyncloader',  be_themes_get_template_directory_uri() . '/js/vendor/asyncloader' . $suffix . '.js', array( 'jquery' ), '1.0' , true );
		wp_enqueue_script( 'asyncloader' );	

		wp_register_script( 'webfontloader',  be_themes_get_template_directory_uri() . '/js/vendor/webfont' . $suffix . '.js' );
		wp_enqueue_script( 'webfontloader' );	
	
		wp_register_script( 'custom-scrollbar',  be_themes_get_template_directory_uri() . '/js/vendor/perfect-scrollbar.jquery' . $suffix .  '.js', array( 'jquery' ), false , true );
		wp_enqueue_script( 'custom-scrollbar' );	
		
		//wp_register_script( 'be-themes-script-js', be_themes_get_template_directory_uri() . '/js/script.js', array( 'jquery','jquery-ui-core','jquery-ui-widget','jquery-ui-mouse','jquery-ui-position','jquery-ui-draggable','jquery-ui-resizable','jquery-ui-selectable','jquery-ui-sortable','jquery-ui-accordion','jquery-ui-tabs','jquery-effects-core','jquery-effects-blind','jquery-effects-bounce','jquery-effects-clip','jquery-effects-drop','jquery-effects-explode','jquery-effects-fade','jquery-effects-fold','jquery-effects-core','jquery-effects-pulsate','jquery-effects-scale','jquery-effects-shake','jquery-effects-slide','jquery-effects-transfer','be-theme-plugins-js','be-main-plugins-js'), FALSE, TRUE );
		wp_register_script( 'be-themes-script-js', be_themes_get_template_directory_uri() . '/js/script' . $suffix . '.js', array( 'jquery', 'asyncloader'), '5.0', true );
		wp_enqueue_script( 'be-themes-script-js' );

		$vendor_scripts_url = ( isset( $be_themes_data[ 'cdn_address' ] ) && !empty( $be_themes_data[ 'cdn_address' ] ) ) ? ( trailingslashit( $be_themes_data[ 'cdn_address' ] ) . 'wp-content/themes/' . get_template() . '/js/vendor/' ) : be_themes_get_template_directory_uri().'/js/vendor/';
		...
		wp_localize_script(
			...
			'oshineThemeConfig', 
			array(
				'vendorScriptsUrl' => ( isset( $be_themes_data[ 'cdn_address' ] ) && !empty( $be_themes_data[ 'cdn_address' ] ) ) ? ( trailingslashit( $be_themes_data[ 'cdn_address' ] ) . 'wp-content/themes/' . get_template() . '/js/vendor/' ) : be_themes_get_template_directory_uri().'/js/vendor/',
...
function oshine_enqueue_to_tatsu_frame() {
	...
	$parsed_to_typehub = get_option( 'oshine_redux_to_typehub' );
	wp_enqueue_style( 'oshine-tatsu-frame-css', be_themes_get_template_directory_uri().'/css/admin/oshine-tatsu-frame.css' );
...
function be_themes_oshine_redux_styles() {
	wp_enqueue_style( 'oshine-redux-styles', be_themes_get_template_directory_uri().'/css/admin/oshine-redux-styles.css' );
}
...
function be_themes_oshine_redux_scripts() {
	wp_enqueue_script( 'oshine-redux-scripts', be_themes_get_template_directory_uri().'/js/admin/oshine-redux-script.js' );
}
...
function oshine_register_custom_font() {
	$font = array(
		'name' => 'Hans Kendrick',
		'src' => be_themes_get_template_directory_uri().'/fonts/hans-kendrick.css',
		'variants' => array(
			'300' => 'Light',
			'400' => 'Normal',
			'500' => 'Medium',
  ...
  $hkgrotesk = array(
      'name' => 'HK Grotesk',
      'src' => be_themes_get_template_directory_uri().'/fonts/hk-grotesk.css',  // URL of your font's stylesheet. Assuming its inside the fonts folder of your child or main theme
      'variants' => array(
          '300' => 'Light',
          '400' => 'Regular',
          '500' => 'Medium',
  ...
  $proximaNova = array(
    'name' => 'Proxima Nova',
    'src' => be_themes_get_template_directory_uri().'/fonts/proxima-nova.css',  // URL of your font's stylesheet. Assuming its inside the fonts folder of your child or main theme
    'variants' => array(
        '300' => 'Light',
        ...
        '500' => 'Semi Bold',
        '500italic' => 'Semi Bold Italic',
        '600' => 'Bold',
        '600italic' => 'Bold Italic',
        '700' => 'Extra Bold',
        '700italic' => 'Extra Bold Italic',
        '800' => 'Black',
        '800italic' => 'Black Italic'
    )
  );
  typehub_register_font( $proximaNova );
