...
module.exports = async (event, context) => {
  ...
  try {
    ...
    // Load hashtag data from request
    data = JSON.parse(event.body);

    // Build the hashtag object
    ...
    if (errs) {
      return createErrorResponse(400, errs.message || 'Invalid hashtag object');
    }
    ...
    return createJsonResponse({ body: hashtag });
  } catch (error) {
    // Log the error
