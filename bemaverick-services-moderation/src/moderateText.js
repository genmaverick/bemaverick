...
const rejectString = "*****";
const textAppId = config.cleanspeak.textAppId || null;
...
const moderateText = async params => {
  ...
  try {
    ...
    const modContent = await cleanSpeakApi.moderateText(
...
const generateContentObj = (params, text, applicationId = null) => {
  ...
  //   from = attr.uuid || from;
  // }
  console.log("process.env.TEXT_APP_ID", textAppId);
  return {
    content: {
      applicationId: applicationId || textAppId,
