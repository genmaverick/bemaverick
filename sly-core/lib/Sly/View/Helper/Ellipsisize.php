...
class Sly_View_Helper_Ellipsisize
{
    /**
	...
	public function ellipsisize($selected, $listItems, $maxItemCount, $itemKeyPrefix = "pageNav-") {
		...
		$windowIndices = $this->getItemEllipsisWindowIndices(substr($selected, strlen($itemKeyPrefix)), 
															 $lastItemIndex, 
															 $maxItemCount);
		...
		$prevHidden = false;
		foreach ($listItems as $key => &$listItem) {
			$ellipsisClasses = (array_key_exists('itemAttributes', $listItem) &&
                                array_key_exists('class', $listItem['itemAttributes'])) ? array($listItem['itemAttributes']['class']) : array();
			...
			if (!$this->isItemVisible($itemIndex, 
				...
				if (!$prevHidden) {
					$ellipsisClasses[] = 'ellipsis';
					$prevHidden = true;
				}
				else {
					$ellipsisClasses[] = 'hidden';		  
				}
			}
			else {
				$prevHidden = false;
			...
			list($itemWinL, $itemWinU) = $this->getItemEllipsisWindowIndices($itemIndex, 
																			 $lastItemIndex, 
																			 $maxItemCount);
     ...
     * @return  boolean
     */
    public function isItemVisible ($itemIndex, $lastItemIndex, $selectedWindowLowerBoundIndex, $selectedWindowUpperBoundIndex) {
     ...
     *                             need ellipses
     */
    public function getItemEllipsisWindowIndices ($itemIndex, $lastItemIndex, $maxItemCount) {
