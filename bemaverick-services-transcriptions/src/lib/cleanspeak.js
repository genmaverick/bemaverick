...
module.exports.moderateText = async (params) => {
  try {
    const applicationId = cleanspeak.responseAppId;
    const content = {
      // todo: add check for content type response/challenge for applicationId var
      applicationId,
      createInstant: Date.now(params.created),
      ...
      senderDisplayName: params.user.username,
      senderId: params.user.userUuid,
    };

    const modContent = await this.moderateContent(
      {
        content,
      },
      params.contentUuid,
