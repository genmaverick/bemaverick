...
// call php api response/updatestatus
...
// call php api response/updatestatus
module.exports.updateResponseStatus = updateResponseStatus = (responseId, status) => {
  return new Promise(async (resolve, reject) => {
    ...
    const options = {
      uri: `${config.maverick.url}/v1/response/updatestatus`,
      method: "GET",
      qs: params,
      json: true,
      headers,
...
// call php api response/updatetranscription
module.exports.updateResponseTranscription = updateResponseTranscription = (responseId, transcriptionText) => {
  return new Promise(async (resolve, reject) => {
    // get new token
    const newToken = await getToken();
    const headers = {
    ...
    const options = {
      uri: `${config.maverick.url}/v1/response/updatetranscription`,
      method: "POST",
      qs: params,
      json: true,
      headers,
...
module.exports.getImage = getImage = (token, imageId, clientToken = false) =>
  new Promise(async (resolve, reject) => {
    if (imageId < 1) {
      console.log(
        `warning: comments.lib.maverick.imageId invalid (${imageId})`
      );
      resolve(null);
    }
    ...
    mention =>
      new Promise((resolve, reject) => {
        ...
        getUserDetails(token, { username })
          .then(userDetails => {
            const user = Object.values(userDetails.users).find(
...
module.exports.getUserDetails = getUserDetails = async (
  token,
  query,
  clientToken = false
) => {
