...
const FaqMavericks = ({ contentMavericks }) => {
  const questions = JSON.parse(contentMavericks);
  const titleArr = [];
  const answerArr = [];

  for (let x = 0; x < questions.length; x += 1) {
    const linkString = `#${x.toString()}`;
    titleArr.push((
      <div key={x}>
        <Question question={questions[x].title.rendered} linkTo={linkString} />
      </div>
    ));
    const answerTitle = <a name={x}><H2>{questions[x].title.rendered}</H2></a>;
    const answerContent = (
      <div
...
FaqMavericks.propTypes = {
  contentMavericks: PropTypes.string.isRequired,
};
