...
(function() {
    var Promise = RSVP.Promise,
        FrameGrab = function(user_passed_opts) {
            ...
            this.grab = function(target_container, time, opt_max_size) {
                if (typeof target_container === "string" && (target_container.toLowerCase() === "canvas" || target_container.toLowerCase() === "img")) {
                    target_container = document.createElement(target_container);
                }

                if (
                    !this._is_element(target_container, "img") &&
            ...
            this.grab_now = function(target_container, opt_max_size) {
                ...
                clone_ready.then(
                    function() {
                        this.grab(target_container, options.video.currentTime, opt_max_size).then(deferred.resolve, deferred.reject);
                    }.bind(this)
                );
            ...
            this.make_story = function(type, images, opt_size) {
                ...
                if (!normalized_type || (normalized_type !== "canvas" && normalized_type !== "img" && normalized_type !== "blob")) {
    ...
    FrameGrab.prototype = {
            ...
            if (videoSrc) {
                ...
                (function() {
                    ...
                    for (idx = 0; idx < source_els.length; idx++) {
                        ...
                        if (srcAttr) {
                            currentElement.setAttribute("src", this._uncacheable_url(srcAttr));
                        }
                    }
                }.bind(this)());
            }
        ...
        _draw_specific_frame: function(spec) {
            ...
            this._seek(spec.video, spec.time_in_secs).then(
                function seek_success() {
                    ...
                    if (spec.skip_solids.enabled && this._is_solid_color(spec.video, spec.skip_solids.max_ratio)) {
                        (function() {
        ...
        _is_element: function(el, type) {
            return el != null && el.tagName != null && el.tagName.toLowerCase() === type;
        },
        ...
        _normalize_options: function(user_passed_options) {
            ...
            if (user_passed_options.skip_solids) {
                ...
                options.skip_solids.frames = user_passed_options.skip_solids.frames || options.skip_solids.frames;

                options.skip_solids.secs = user_passed_options.skip_solids.secs || options.skip_solids.secs;

                options.skip_solids.max_ratio = user_passed_options.skip_solids.max_ratio || options.skip_solids.max_ratio;
            }
        ...
        _normalize_time: function(time, frame_rate) {
            ...
            } else if (typeof time === "string" && typeof frame_rate === "number" && frame_rate > 0) {
        ...
        _uncacheable_url: function(url) {
            ...
            if (url.indexOf("blob:") !== 0) {
                return url + param_prefix + "fgtimestamp=" + new Date().getMilliseconds();
            }
